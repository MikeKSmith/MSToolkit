---
layout: page
title: emaxfit R Function
description: R code for fitting Emax, log-linear or linear models in a "step-
down" approach
---
...
SeEmax3 <- function (fit, doselev)
{
  ...
  vfcov <- vcov(fit)
  sddif <- sqrt((fitdif ^ 2) * (
    vfcov[3, 3] / coef(fit)[3] ^ 2 +
      vfcov[1, 1] / (coef(fit)[1] + doselev) ^
      2 - (2 * vfcov[1,
                     3]) /
      (coef(fit)[3] * (coef(fit)[1] + doselev))
  ))
  covint <-
    -((doselev * coef(fit)[3]) / (coef(fit)[1] + doselev) ^ 2) *
